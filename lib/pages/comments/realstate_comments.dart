...
class RealStateComments extends StatefulWidget {
  final int postid;

  const RealStateComments({Key key, @required this.postid}) : super(key: key);

  @override
  _RealStateCommentsState createState() => _RealStateCommentsState();
}

class _RealStateCommentsState extends State<RealStateComments> {
...
print(widget.postid);
    Requests.getRealStateComments(widget.postid).then((value) {
    ...
    return SafeArea(
      child: Scaffold(
        body: Container(
          child: Stack(
            children: [
              loading  ?Center(child:CircularProgressIndicator()):comList.length == 0?Center(child: Text("لا توجد تعليقات"),):
              ...
              Positioned(bottom:5,right: 5,left: 5,child: RButton(txt: "إضافة تعليق", press: () async {
 ...
 void addComment() {
   ...
   double userRating = 5 ;
    ...
    if(commentController.text.isEmpty){
     ...
     Requests.addComments("api/realstate_comments/addnew", rating, commentController.text, widget.postid);
