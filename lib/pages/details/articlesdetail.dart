...
class ArticlesDetail extends StatefulWidget {
  ...
  const ArticlesDetail({Key key, this.postList, this.id = 0}) : super(key: key);
...
class _ArticlesDetailState extends State<ArticlesDetail> {
  ...
  Future<List<ArticleModel>> getArticle(id) async {
    var response = await Constants.getUrl("api/articles/get/$id/");
  ...
  void initState() {
    super.initState();
    if (widget.id != 0) {
      getArticle(widget.id).then((value) {
        setState(() {
          mylist = value;
        });
      });
...
class AddToFavorite extends StatefulWidget {
  final bool pressed;
  final list;
  const AddToFavorite( {Key key, this.pressed = false, this.list}) : super(key: key);
...
class _AddToFavoriteState extends State<AddToFavorite> {
 ...
 Future<String> checkFav(id) async {
    ...
    var response =
        await Constants.getUrl("api/users_favorite/get/$id/article/$user/");
...
if(_pressed == true)
         Requests.addToFavorite(widget.list.id, widget.list.title, widget.list.desc, "article");
else
Requests.removeFromFavorite(widget.list.id, "article");
