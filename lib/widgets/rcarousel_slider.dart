...
class RCarouselSlider extends StatefulWidget {
  ...
  const RCarouselSlider({
    Key key,
    this.imgList, this.list, this.type,
  }) : super(key: key);
...
void initState() { 
  ...
  imageSliders = widget.imgList
        .map((item) {
             ...
             fit: BoxFit.cover,width: width, placeholder: AssetImage("assets/icons/placeholder.jpg",),),
                        Positioned(
                          bottom: 0.0,
                          left: 0.0,
                          right: 0.0,
    ...
    return Container(
      ...
      child:Stack(
        children: [
          Column(children: [
           widget.imgList.length == 0 ? Container(margin: EdgeInsets.symmetric(horizontal: 20),color: Colors.grey.withOpacity(0.7),height: 200) : CarouselSlider(
             items:  imageSliders,
          ...
          Positioned(
            ...
            child: RawMaterialButton(
              ...
              setState(() =>_pressed = !_pressed);
                if(_pressed == true)
                Requests.addToFavorite(widget.list.id, widget.list.title, widget.list.desc, widget.type);
                else
